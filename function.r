...
#Y: trait,n length vector; Mom, Dad and Baby: genotype matrix n*m ; X: covariate matrix, n*p
FB_GGRF<-function(Y,Mom,Dad,Baby,X=NULL,kernel='IBS',centerS=F,weights=1,type='normal',od=3) 
{
  Z<-(Mom+Dad)/2-Baby;
  ...
  #Cauculate p-value
  p.value<-davies(0,eigen(M1,symmetric=TRUE,only.value=TRUE)$values)$Qq
...
InverseNorm<-function(x,group=1)
{
	ng<-length(unique(group));z<-x;
	for(i in 1:ng)
  {
		pos<-rep(group,length=length(x))==unique(group)[i];
		R<-rank(x[pos]);
		z[pos]<-qnorm((R+0.5)/sum(pos))
	}
	z;
}
